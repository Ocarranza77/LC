			    ...
			    success: function (response) {
			    	...
			    	if(parseInt(result[0])==0){
						...
						if($('#container_history > li').eq(indexList).children('ul .row_ID').children('li').children('input').val()!="ST2" ){
							...
							$('#container_history > li').eq(indexList).children('ul').children('li .column_sttReg').children('img').attr('title',result[1]);
			    		...
			    		//alert(response.d);
			    		$('#container_history > li').eq(indexList).children('ul').children('li .column_sttReg').children('img').attr('src','../../iconos/exclamation.png');
						$('#container_history > li').eq(indexList).children('ul').children('li .column_sttReg').children('img').attr('title',result[1]);
						msgText += "\n"+result[1];
...
function ADDRow(event){
	...
	var html="<li class='rowM'><ul >";
		...
		html +="</select></li>";
		html +="<li class='column_FechaT' ><input value='"+$("#txtFecha").val()+"' onkeyup='this.value=formateafecha(this.value);' maxlength='10' placeholder='dd/mm/aaaa' /></li>";
		...
		html +="<li class='column_FolioT'><input onkeyup='verificar(event);'  onkeypress='return justNumbers(event);' /></li>";
  			...
  			if($(event.target).attr('src')=='../../iconos/sign_down.png'){/* condicion si esta habilitado para agrgar  al daily cash sumary*/
				...
				$('#container_history1 > li').each(function (index) {
					...
					if(_SucId+""+_FechaZ == SucId+""+FDaily){
						...
						$('#container_history1 > li').eq(_indexActual).children('ul').children('li .column_TipoCambio').children('input').keyup();
  ...
  function AddDaily(ArrayDatos){
	var html ="<li class='rowM'>";
		...
		*/
		...
		$('#container_history1 > li').eq(1).children('ul').children('li .column_TipoCambio').children('input').keyup();
...
*/
  ...
  function VerComent(event){
	...
	var comentario=$('#container_history1 > li').eq(index1).children('ul').children('li .column_coment').children('input').val();
	

	//$("#textcoment").text(comentario);
	document.getElementById("textcoment").value=comentario;

  	$("#WindowComent").css({'top': $(event.target).position().top+(parseFloat($("#WindowComent").height())/2) });
  	$("#WindowComent").css({'left': $(event.target).position().left-($("#WindowComent").width()+10)});
