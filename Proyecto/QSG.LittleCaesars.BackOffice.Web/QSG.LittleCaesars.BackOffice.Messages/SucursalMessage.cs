...
namespace QSG.LittleCaesars.BackOffice.Messages
{
    public partial class ServiceImplementation
    {
        ...
        public SucursalResponse SucursalMessage(SucursalRequest request)
        {
            ILog _log4net = LogManager.GetLogger("SucursalMessage");

            var response = new SucursalResponse();
            var bl = new SucursalBL();
            ...
            try
            {
                ...
                {
                    response.Sucursales = bl.GetSucursales(request.Sucursal, request.UserIDRqst, ref msg);
                    response.FriendlyMessage = msg;
                }
